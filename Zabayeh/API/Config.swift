//
//  Config.swift
//  Zabayeh
//
//  Created by endpoint on 5/16/20.
//  Copyright © 2020 endPoint. All rights reserved.
//

import Foundation
let mainURL = "http://zbayeh.com/"
let sliderURL = mainURL + "api/slider"
let imageURL = mainURL + "public/uploads/"
let categoriesURL = mainURL + "api/categories"
let productsURL = mainURL + "api/products"
let offersURL = mainURL + "api/offers"
let subOffersURL = mainURL + "api/offer"
let loginURL = mainURL + "api/login"
let registerURL = mainURL + "api/register"
let creatnewOrderURL = mainURL + "api/create-order"
let settingsURL = mainURL + "api/settings"
let contactUsURL = mainURL + "api/contact-us"
let updateProfileURL = mainURL + "api/update"
let MyOrdersURL = mainURL + "api/orders"
let balanceURL = mainURL + "api/get-balance"
